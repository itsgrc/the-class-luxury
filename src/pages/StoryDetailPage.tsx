...
import { ArrowLeft, Clock, Calendar, Share2, Bookmark, BookmarkCheck, ChevronUp } from 'lucide-react'
...
export function StoryDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FDF9F2] pb-20">
      <Helmet>
        ...
        <motion.div ref={contentRef} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <div className="space-y-5">
            {paragraphs.map((para, i) => (
              <p key={i} className={`text-[#5A4F44] font-light leading-[1.9] text-[15px] ${i === 0 ? 'text-[16px] font-normal' : ''}`}>
                {para}
              </p>
            ))}
          </div>
